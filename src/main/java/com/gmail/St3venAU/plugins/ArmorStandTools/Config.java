    ...
    private static ItemStack toItemStack(String s) {
        ...
        if (split.length == 2) {
            try {
                ...
                plugin.getLogger().warning("Error in config.yml: Invalid data value specified. Continuing using data value 0 instead.");
            }
        }
        Material m;
        try {
            ...
            plugin.getLogger().warning("Error in config.yml: Invalid material name specified. Continuing using AIR instead.");
            return new ItemStack(Material.AIR);
        }
